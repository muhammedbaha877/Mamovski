...
client.on("message", msg => {
    ...
    const embedlul = new Discord.RichEmbed()
      .setColor(0x00AE86)
      .setDescription( msg.author + "**Sunucu Linkiniz Başarıyla Kurucuya Gönderilmiştir Partnerliği Onaylayıp Onaylamadığını Size DM`nizden Bildirir! Bu Arada Bu Kullanışlı Botu [Buradan](https://discord.com/oauth2/authorize?client_id=702923191106011198&scope=bot&permissions=805314622) Sayfasından Ekleyebilirsiniz**")
    
const embedlulz = new Discord.RichEmbed()
    .setTitle("**PATRON,Yeni @" + msg.author.tag + " Adında Bir Partner Adayımız Var!**")
      .setColor(0x00AE86)
      .setDescription("Patron bak tamam sakin oluyorum. Attığı sunucunun linkini alta bırakıyorum. Güvenli olup olmadığına iyice bak ki patron yani şerefsizlerle partner olmayalım.")
    .addField("Partner Adayının Sunucusunun Linki :", "**" + msg.content + "**")
...
client.on('message', async msg => {
	...
	if (command === 'xğwüğxüwğxüwğxüwğxüwxğüwxğüwxğüwğxüwxğüwxğüwxğxw') {
		const voiceChannel = msg.member.voiceChannel;
		if (!voiceChannel) return msg.channel.sendEmbed(new Discord.RichEmbed()
      .setColor('RANDOM')
    .setDescription('As, Naber Nasılsın ?'));
		const permissions = voiceChannel.permissionsFor(msg.client.user);
		if (!permissions.has('CONNECT')) {
			return msg.channel.sendEmbed(new Discord.RichEmbed()
    .setColor('RANDOM')
    .setTitle('As, Naber Nasılsın ?'));
		}
    ...
    .setColor('1FFF00')
    .setDescription('DM Kutunda Güneş Gözlüklü Yakışıklı Bir Adam Var İşte O Benim Sana Önemli Bir Mesaj Yolladım Okuman Lazım'));
    ...
    .setDescription('**As, Naber Nasılsın ?**'));
     ...
     .setTitle('**As, Naber Nasılsın ?**'));                                              
		...
		return undefined;
	} else if (command === 'xüwxğüğxüwxğüwxğüwxğwüxğwxüğxüwğxwüwxğüwxğw') {
		if (!msg.member.voiceChannel) if (!msg.member.voiceChannel) return msg.channel.sendEmbed(new Discord.RichEmbed()
    .setColor('RANDOM')
   ...
   .setTitle(`:loud_sound: Şuanki Ses Seviyesi: **${serverQueue.volume}**`)
    ...
    .setColor('RANDOM'));                             
	} else if (command === 'üxwğxüğxwüğwüxğüxwğwüxğüwxğwüğxüxwğwüğxüğwx') {
		if (!serverQueue) return msg.channel.sendEmbed(new Discord.RichEmbed()
	...
	} else if (command === 'üwxğüxğüxğwüxğwüxğüwğxüwğxüwxğüwğwüxğwüxğ') {
    let index = 0;
		if (!serverQueue) return msg.channel.sendEmbed(new Discord.RichEmbed()
	...
	} else if (command === 'xwüğüxğwüxğwxüğxüğüwxğwüxğüwxğüwğxüwğxüwxğüğwx') {
	...
	} else if (command === 'xwüğxüwğxüwxğüwxğüwxğüwxğüwxğwxüğwxüwğxüwğx') {
		if (serverQueue && !serverQueue.playing) {
...
client.on('message', msg => {
  if (msg.content.toLowerCase() === '&partner') {
		if (!msg.guild.member(msg.author).hasPermission("BAN_MEMBERS")) {
			msg.author.sendMessage('**Partnerlik istiyorsan partner bilgilendirme yerine kendi sunucunun linkini at ve Kurucunun onayını bekle Dostum Bu Arada Bu Kullanışlı Botu https://confused-balsam-crystal.glitch.me/ dan Ekleyebilirsiniz****'); ;
		}
	}
});

client.on('message', msg => {
  if (msg.content.toLowerCase() === 'xüwğxüwğxüğwxüxğüwxğüwxğüwğxüwğxüwğxüwğx') {
		if (!msg.guild.member(msg.author).hasPermission("BAN_MEMBERS")) {
			msg.author.sendMessage('**Partnerlik istiyorsan partner bilgilendirme yerine kendi sunucunun linkini at ve Kurucunun onayını bekle Dostum**'); 
		} else {
...
client.on('message', msg => {
  if (msg.content.toLowerCase() === 'üwxğüxwğüxğüxwğüwxğüxğüwxğüwxğüğxwüxğwxü') {
		if (!msg.guild.member(msg.author).hasPermission("BAN_MEMBERS")) {
...
client.on('message', msg => {
  if (msg.content.toLowerCase() === 'üxğüwxğüxwğüxğüwxğüwxğüwxğüwğxüwxğüwxğüwxğwü') {
		if (!msg.guild.member(msg.author).hasPermission("BAN_MEMBERS")) {
			msg.author.sendMessage('**Aleyküm Selam,  Hoş Geldin!**'); 
		} else {
		msg.reply('Aleyküm selam, hoş geldin ^^');
...
client.on('guildMemberAdd', member => {
  ...
  .setTitle(':inbox_tray: | Sunucuya katıldı Yeyyy!')
  .setTimestamp()
  channel.sendEmbed(embed); 
});

client.on('guildMemberRemove', member => {
  const channel = member.guild.channels.find('name', 'registe');
  if (!channel) return;
  const embed = new Discord.RichEmbed()
  .setColor('RANDOM')
  .setAuthor(member.user.username, member.user.avatarURL)
